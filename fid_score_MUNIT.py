...
parser.add_argument('path', type=str, nargs=2,
                    help='give two paths, first one to the MUNIT image folder (with sub-folders), the second one to the test data folder')
parser.add_argument('--batch-size', type=int, default=50, help='Batch size to use')
parser.add_argument('--dims', type=int, default=192, choices=list(InceptionV3.BLOCK_INDEX_BY_DIM), help='Dimensionality of Inception features to use. ' 'By default 192')
...
def _compute_statistics_of_path_munit(path, model, batch_size, dims, cuda):
    path = pathlib.Path(path)
    files = list(path.glob('*/*.jpg')) + list(path.glob('*/*.png'))
    m, s = calculate_activation_statistics(files, model, batch_size,
                                           dims, cuda)
...
def calculate_fid_given_paths(paths, batch_size, cuda, dims):
    ...
    m1, s1 = _compute_statistics_of_path_munit(paths[0], model, batch_size,
                                         dims, cuda)
